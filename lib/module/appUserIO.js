...
//(function(){
	...
	// This is a modified version to work as one of the dependencies
    angular.module('UserAppIO', [])

    // Expose the UserAppIO API
    .value('UserAppIO', UserApp)

    // Authentication service
    .factory('user', function($rootScope, $route, $location) {
            var user = {};

		    //Normally is set from Init, but we have merge into the base module and access is more complex
			//Therefore we set it here; this is a account ID @ userapp.io
			var appId = '52c994cd601c5';
    ...
    // Logout directive
    .directive('uaLogout', function(user) {
            return {
                    restrict: 'A',
    ...
    // Login directive
    .directive('uaLogin', function($rootScope, user) {
            return {
                    restrict: 'A',
                    link: function(scope, element, attrs) {
    ...
    // Signup directive
    .directive('uaSignup', function($rootScope, user, UserApp) {
        return {
            restrict: 'A',
            link: function(scope, element, attrs) {
                element.on('submit', function(e) {
    ...
    // OAuth URL directive
    .directive('uaOauthLink', function(UserApp) {
        return {
            restrict: 'A',
            link: function(scope, element, attrs) {
                element.on('click', function(e) {
    ...
    .directive('uaHasPermission', function(user) {
        return {
            restrict: 'A',
            link: function(scope, element, attrs) {
    ...
    .directive('uaHasFeature', function(user) {
        return {
            restrict: 'A',
            link: function(scope, element, attrs) {
                element[0].style.display = 'none';
